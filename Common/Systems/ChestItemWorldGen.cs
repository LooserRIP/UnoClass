...
namespace UnoClass.Common.Systems
{
	...
	public class ChestItemWorldGen : ModSystem
	{
		...
		public override void PostWorldGen() {
			...
			int[] itemsToPlaceInLivingChests = { ModContent.ItemType<LivingDeck>() };
			...
			for (int chestIndex = 0; chestIndex < Main.maxChests; chestIndex++) {
				...
				if (chestTile.TileType == TileID.Containers && chestTile.TileFrameX == 12 * 36) {
					...
					for (int inventoryIndex = 0; inventoryIndex < Chest.maxItems; inventoryIndex++) {
						if (chest.item[inventoryIndex].type == ItemID.None) {
							// Place the item
							chest.item[inventoryIndex].SetDefaults(itemsToPlaceInLivingChests[itemCycle]);
							// Decide on the next item that will be placed.
							itemCycle = (itemCycle + 1) % itemsToPlaceInLivingChests.Length;
				...
				if (maxItems != -1 && itemsPlaced >= maxItems) {
					break;
				}
			}
		}
